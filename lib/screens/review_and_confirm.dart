...
void _setCheckboxValue(String key, bool value) {
    ...
    return await showDialog<bool>(
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              ...
              content: Text('Are you sure you want to delete this $sectionType?'),
              actions: [
                TextButton(
      ...
      child: Row(
        children: [
          CircleAvatar(
            ...
            child: Text(
              (user.givenNames?.isNotEmpty == true && user.surname?.isNotEmpty == true)
      ...
      children: [
        ...
        if (user.educationQualifications?.isNotEmpty == true)
          _buildSection(
            ...
            Colors.green,
            () => _showEducationQualificationsDialog(user.educationQualifications!),
          ),
  ...
  Widget _buildSection(String title, IconData icon, int count, Color color, VoidCallback onTap) {
    return Container(
  ...
  // Dialog methods for each section (now only with delete functionality)
   void _showPersonalDetailsDialog(List<PersonalDetails> details) {
    _showItemsDialog(
      ...
      (item) => _deleteFormSection('Personal Details', item.personalDetailsId ?? 0),
      (item) => _savePersonalDetails(item), // Add save method
    );
  }

  void _showEducationQualificationsDialog(List<EducationQualifications> qualifications) {
  _showItemsDialog(
    'Education Qualifications',
    qualifications,
    (item, isEditing) => _buildEducationQualificationItem(item, isEditing),
    (item) => _deleteFormSection('Education Qualifications', item.educationId ?? 0),
...
void _showLicencesDialog(List<Licences> licences) {
 ...
 void _showPriorityProcessDialog(List<PriorityProcess> processes) {
  ...
  void _showItemsDialog<T>(
    ...
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return StatefulBuilder( // Changed to StatefulBuilder for local state
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Container(
                            ...
                            child: Column(
                              children: [
                                itemBuilder(item, isEditingMode), // Pass editing state
                                _buildActionButtons(
                                  onDelete: () => onDelete(item),
                                  onSave: isEditingMode ? () => onSave(item) : null,
 ...
 Widget _buildEducationQualificationItem(EducationQualifications item, bool isEditing) {
  return Padding(
    padding: const EdgeInsets.all(16),
    child: Column(
      crossAxisAlignment: CrossAxisAlignment.start,
...
Widget _buildOccupationItem(UserOccupations item, bool isEditing) {
 ...
 Widget _buildPriorityProcessItem(PriorityProcess item, bool isEditing) {
  ...
  Widget _buildEditableInfoRow(String label, String value, String controllerKey, bool isEditing) {
  ...
  Future<void> _savePersonalDetails(PersonalDetails item) async {
    ...
    final updatedItem = PersonalDetails(
      ...
      givenNames: _getController('given_names_${item.personalDetailsId}', '').text,
      ...
      countryOfBirth: _getController('country_birth_${item.personalDetailsId}', '').text,
...
Future<void> _saveEducationQualification(EducationQualifications item) async {
  final updatedItem = EducationQualifications(
    ...
    qualificationName: _getController('qual_name_${item.educationId}', '').text,
    majorField: _getController('major_field_${item.educationId}', '').text,
    institutionName: _getController('institution_name_${item.educationId}', '').text,
    institutionCountry: _getController('institution_country_${item.educationId}', '').text,
    courseStartDate: _getController('course_start_${item.educationId}', '').text,
...
Future<void> _saveEducation(Educations item) async {
  final updatedItem = Educations(
    ...
    dateFinished: _getController('date_finished_${item.educationId}', '').text,
    numberOfYears: int.tryParse(_getController('num_years_${item.educationId}', '').text),
    certificateDetails: _getController('cert_details_${item.educationId}', '').text,
  );
...
Future<void> _saveEmployment(Employments item) async {
  final updatedItem = Employments(
    ...
    isapplicantemployed: _getCheckboxValue('currently_employed_${item.id}', false),
  );
...
Widget _buildEditableCheckboxRow(String label, bool value, String key, bool isEditing) {
  return Padding(
    padding: const EdgeInsets.only(bottom: 12),
    child: Row(
      children: [
