...
Widget _buildVerticalSearch() {
 ...
 Widget _buildApplicationSteps(BuildContext context) {
    ...
    return Padding(
      ...
      child: isMobile ? _buildMobileLayout(context) : _buildDesktopLayout(context, connectorWidth),
    );
  }
  ...
  Widget _buildMobileLayout(BuildContext context) {
    final steps = [
      _StepData('1', 'Nominate', 'Nominate an occupation', ' Check the qualification and employment requirements for your occupation.'),
      _StepData('2', 'Check', 'Check your occupation description.', ' View the Professional Occupations list to understand whether your work experience matches the occupation description.'),
      _StepData('3', 'Prepare', 'Prepare for your application. Still unsure about the application criteria or documents? We have some optional services. ', null, [
      ...
      child: Column(
        ...
        children: [
          Row(
            crossAxisAlignment: CrossAxisAlignment.center,
            children: [
              _buildNumberBox(context, step.number, screenWidth * 0.15), // 15% of screen width
              const SizedBox(width: 16),
              Expanded(
                child: Text(
                  step.title,
    ...
    return SingleChildScrollView(
      ...
      child: IntrinsicHeight(
        child: Row(
          ...
          children: [
            // Step 1
            _buildDesktopStep(context, '1', 'Nominate', 'Nominate an occupation', ' Check the qualification and employment requirements for your occupation.'),
            _buildHorizontalConnector(context, connectorWidth),
            
            // Step 2
            _buildDesktopStep(context, '2', 'Check', 'Check your occupation description.', ' View the Professional Occupations list to understand whether your work experience matches the occupation description.'),
            _buildHorizontalConnector(context, connectorWidth),
            
            // Step 3
            _buildDesktopStep(context, '3', 'Prepare', 'Prepare for your application. Still unsure about the application criteria or documents? We have some optional services. ', null, [
              _LinkData('Get Skills Assessment Support.', true),
              _LinkData(' Find out about ', false),
              _LinkData('Priority Processing', true),
              _LinkData(' to fast track your application.', false),
            ...
            // Step 4
            _buildDesktopStep(context, '4', 'Apply', 'Apply now. You can ', null, [
              _LinkData('apply online.', true),
            ]),
          ],
        ),
  ...
  Widget _buildDesktopStep(BuildContext context, String number, String title, String mainText, String? additionalText, [List<_LinkData>? links]) {
    final screenWidth = MediaQuery.of(context).size.width;
    final isTablet = screenWidth >= 768 && screenWidth < 1024;
    ...
    return SizedBox(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildStepDescription(context, _StepData(number, title, mainText, additionalText, links)),
        ],
      ),
    );
  }
  ...
  Widget _buildNumberBox(BuildContext context, String number, double size, {double? fontSize}) {
    return Container(
      width: size,
      height: size,
      decoration: BoxDecoration(
    ...
    if (step.links != null) {
      for (_LinkData link in step.links!) {
        spans.add(TextSpan(
          text: link.text,
          style: TextStyle(
            color: link.isLink ? const Color(0xFF006064) : const Color(0xFF5A5A5A),
            fontWeight: link.isLink ? FontWeight.bold : FontWeight.normal,
            decoration: link.isLink ? TextDecoration.underline : TextDecoration.none,
            fontSize: fontSize,
          ),
        ));
      }
...
class _StepData {
  ...
  _StepData(this.number, this.title, this.mainText, this.additionalText, [this.links]);
}
